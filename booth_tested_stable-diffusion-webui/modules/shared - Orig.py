...
## A custom function created to process all files within all folders in the input directory and then replicates them later on
## FIX ME: DEFINE THIS FUNCTION PROPERLY
def listfilesrecur(dirname):

    filenames = [os.path.join(dirname, x) for x in sorted(os.listdir(dirname), key=natural_sort_key) if not x.startswith(".")]
    file_lst = []
    ## loop through all items, if it is a file, append to file_lst, else if folder, rescurse into folder
    for i in filenames:
        if (os.path.isfile(i)):
            file_lst.append(i)
        elif (os.path.isdir(i)):
            #file_lst.append(listfilesrecur(i))
            [file_lst.append(file) for file in listfilesrecur(i)]
        else:
            print ('found invalid file/folder')
            continue;

    #return [file for file in filenames if os.path.isfile(file) else listfilesrecur(file)]
    return file_lst

### Custom Code ends here --Shahbaz
