...
class StableDiffusionProcessing:
    ...
    def __init__(self, sd_model=None, outpath_samples=None, outpath_grids=None, prompt: str = "", styles: List[str] = None, seed: int = -1, subseed: int = -1, subseed_strength: float = 0, seed_resize_from_h: int = -1, seed_resize_from_w: int = -1, seed_enable_extras: bool = True, sampler_name: str = None, batch_size: int = 1, n_iter: int = 1, steps: int = 50, cfg_scale: float = 7.0, width: int = 512, height: int = 512, restore_faces: bool = False, tiling: bool = False, do_not_save_samples: bool = False, do_not_save_grid: bool = False, extra_generation_params: Dict[Any, Any] = None, overlay_images: Any = None, negative_prompt: str = None, eta: float = None, do_not_reload_embeddings: bool = False, denoising_strength: float = 0, ddim_discretize: str = None, s_min_uncond: float = 0.0, s_churn: float = 0.0, s_tmax: float = None, s_tmin: float = 0.0, s_noise: float = 1.0, override_settings: Dict[str, Any] = None, override_settings_restore_afterwards: bool = True, sampler_index: int = None, script_args: list = None):
        ...
        self.outpath_samples: str = (outpath_samples[:125]).strip()
        ...
        # for getting runtime of program
        import time;
        init_time = time.strftime("%H:%M:%S");
        print (f"start time: ", init_time);

        # this code is to log the prompts in a text file located in the directory of samples
        is_created = 0
        counter = 1
        if (not(os.path.exists((self.outpath_samples).strip()))):
            os.mkdir(self.outpath_samples)

        while (is_created != 1):
            try:
                with open ((self.outpath_samples).strip()+"/prompt " + str(counter) + ".txt", 'x') as f:
                    f.write ("Postive Prompt:\n" + str(self.prompt) + "\nNegative Prompt:\n" + str(self.negative_prompt)+ "\nSeed:\n" + str(self.seed) + "\nnumber of images generated: " + str(self.n_iter * self.batch_size))
                    is_created = 1;
                    print ("Creating text file for logging prompt...")
            except FileExistsError:
                print ("File already exists! Trying to rename file to log prompt...")
                counter = counter + 1
                continue
